...
# 对水印图像进行同态加密
encrypted_watermark = encrypt_image(watermark_image)

# 定义一个函数对单通道进行水印嵌入，先加密cH2，再嵌入加密的水印
def embed_encrypted_watermark(channel, watermark_image, alpha=0.1, iv=None):
    coeffs = pywt.wavedec2(channel, 'haar', level=2)
    cH2, cV2, cD2 = coeffs[2]

    # 对 cH2 进行同态加密
    cH2_encrypted = encrypt_image(cH2)

    # 将水印调整为与高频系数相同的大小
    watermark_resized = cv2.resize(watermark_image, (cH2.shape[1], cH2.shape[0]))

    # 嵌入加密后的水印
    encrypted_cH2_with_watermark = np.frombuffer(cH2_encrypted, dtype=np.uint8) + alpha * watermark_resized.flatten()
    
    # 保证合并后的数据正确
    encrypted_cH2_with_watermark = np.clip(encrypted_cH2_with_watermark, 0, 255)

    # 解密加密后的cH2与水印
    decrypted_cH2_with_watermark = decrypt_image(encrypted_cH2_with_watermark.tobytes(), iv, cH2.shape)

    # 更新小波系数
    coeffs[2] = (decrypted_cH2_with_watermark, cV2, cD2)
...
# 对每个通道分别嵌入加密的水印
b_channel_watermarked = embed_encrypted_watermark(b_channel, encrypted_watermark, iv=iv)
g_channel_watermarked = embed_encrypted_watermark(g_channel, encrypted_watermark, iv=iv)
r_channel_watermarked = embed_encrypted_watermark(r_channel, encrypted_watermark, iv=iv)
